...
const { Add } = require("midgard-yarn-lib/lib/cli/commands/add");
const { Install } = require("midgard-yarn-lib/lib/cli/commands/install");
const {
  cleanLockfile,
} = require("midgard-yarn-lib/lib/cli/commands/upgrade");
const Config = require("midgard-yarn-lib/lib/config").default;
const { EventReporter } = require("midgard-yarn-lib/lib/reporters");
const Lockfile = require("midgard-yarn-lib/lib/lockfile").default;
const parse = require("midgard-yarn-lib/lib/lockfile/parse").default;
...
async function updateDependencyFiles(directory, dependencies) {
  ...
  const runTime ={"RunTime":Date.now() - start};
  updateRunResults = Object.assign(updateRunResults, runTime);
...
async function updateDependencyFile(
  ...
  const enableTime = {"Enable lock file versions": Date.now() - start};

  start = Date.now();
  const lockfile = await Lockfile.fromDirectory(directory, reporter);
  const getLockFile = {"Get lock file": Date.now() - start};
  ...
  const manifestTime = {"Read root manifest taken": Date.now() - start};

  start = Date.now();
  
  const existingVersionRequirement = flattenAllDependencies(manifest)[depName];
  const flatten = {"Flatten all deps": Date.now() - start};
  ...
  await add.init();
  const addTime = {"AddTime":Date.now() - start};

  start = Date.now();
  const dedupedYarnLock = fixDuplicates(readFile("yarn.lock"), depName);
  const dedupeTime = {"Dedupe Time":Date.now() - start}
  ...
  const write = {"Write file async": Date.now() - start};
